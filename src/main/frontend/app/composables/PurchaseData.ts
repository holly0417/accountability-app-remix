...
export function purchaseData() {
    ...
    const getCurrentUserPurchaseHistory = async (page: number = 0, size: number = 50): Promise<Page<PurchaseDto>> => {
    ...
    const getCurrentUserPurchaseListByStatus = async (status: PurchaseStatus, page: number = 0, size: number = 50): Promise<Page<PurchaseDto>> => {
    ...
    const getPurchaseListByStatusAndUserId = async (usersById: number[], status: PurchaseStatus): Promise<Page<PurchaseDto>> => {

        return (await api.get<Page<PurchaseDto>>('/purchase', {
            params: {
                userIds: usersById, status: status
            }, paramsSerializer: {
                indexes: null
            }
        })).data;
    ...
    const getPurchaseListByUserIds = async (usersById: number[]): Promise<Page<PurchaseDto>> => {
