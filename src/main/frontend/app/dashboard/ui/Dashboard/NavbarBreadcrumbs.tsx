...
const StyledBreadcrumbs = styled(Breadcrumbs)(({theme}) => ({
    margin: theme.spacing(1, 0), [`& .${breadcrumbsClasses.separator}`]: {
        color: (theme.vars || theme).palette.action.disabled, margin: 1,
...
export default function NavbarBreadcrumbs() {
    ...
    const crumbs = matches
        ...
        .map((match) => {
            ...
            const label = typeof handleData.breadcrumb === "function" ? handleData.breadcrumb() // Pass loader data to function
    ...
    return (<StyledBreadcrumbs aria-label="breadcrumb" separator={<NavigateNextRoundedIcon fontSize="small"/>}>
            ...
            {crumbs.map((crumb, index) => {
                ...
                return isLast ? (// Last item is text (not clickable)
                    <Typography key={crumb.path} color="text.primary">
                        {crumb.label}
                    </Typography>) : (// Middle items are links
                    <Link
                        key={crumb.path}
